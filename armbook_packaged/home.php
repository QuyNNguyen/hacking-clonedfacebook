...
<script src="//cdnjs.cloudflare.com/ajax/libs/x-editable/1.5.0/jqueryui-editable/js/jqueryui-editable.min.js"></script>
	...
	// Fix CSFR - Getting token from PHP SESSION
	...
	$( document ).ready(function() {
		...
		// Fix CSFR - sending token
		$( "#add_friend" ).click(function() {
			$.get( "add_friend.php?id=<?php echo $id_to_get; ?>&token="+ token, function( data ) {
			  event.preventDefault();
			});
			location.reload();
		});
		$( "#del_friend" ).click(function() {
			$.get( "del_friend.php?id=<?php echo $id_to_get; ?>&token="+ token, function( data ) {	
			  event.preventDefault();
			});
			location.reload();
		});		
